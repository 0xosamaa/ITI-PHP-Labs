...
$errors=[];
        ...
        if($errors){
            session_start();
            $_SESSION['user_data'] = json_encode($_POST);
            $_SESSION['errors'] = json_encode($errors);
            header("Location: http://localhost:8000/edit_user.php?{$_POST['update_id']}");
            exit();
        }else{
            // TODO look at $new_user datatype
            $update_id = array_pop($_POST);
            $database = file("database.txt");
            $old_user = $database[$update_id];
            unset($database[$update_id]);
            $file = fopen("database.txt","w");
            fwrite($file,implode($database));
            fclose($file);
            
            $new_user = "{$_POST['fname']}:{$_POST['lname']}:{$_POST['username']}:{$_POST['password']}:{$_POST['department']}:{$_POST['code']}:{$_POST['gender']}:{$_POST['address']}:{$_POST['country']}";
