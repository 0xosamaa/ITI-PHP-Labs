...
if (!empty($errors)) {
    ...
    try {
        ...
        $query = "Insert INTO users (name, email, password, room, ext, image) Values(?,?,?,?,?,?)";
        $stmt = $db->prepare($query);
        $stmt->execute([$_POST['name'], $_POST['email'], $_POST['password'], $_POST['room_no'], $_POST['ext'], $_FILES['profile_pic']['name']]);
